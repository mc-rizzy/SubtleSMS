...
public class ChatAdapter extends RecyclerView.Adapter<ChatAdapter.ViewHolder> {
    ...
    // Reuse SimpleDateFormat to avoid memory allocation and GC overhead while scrolling
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        // 1. Efficient Text Binding
        holder.tvBody.setText(msg.getBody());

        // Format time using synchronized/reused Date object to prevent garbage collection pauses
        synchronized (TIME_FORMATTER) {
            reusableDate.setTime(msg.getTimestamp());
            holder.tvTime.setText(TIME_FORMATTER.format(reusableDate));
        }

        // Default state resets for recycled views
        holder.tvBody.setAlpha(1.0f);

        // 2. Optimized Background and Alpha Assignment
    ...
    // Cache Drawables in memory so context lookup and Inflate passes don't block scrolling frames
